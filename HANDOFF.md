...
- WordPress integration checks exercise edit and undo success, nonces, object and field capabilities, allowlists, invalid input, stale snapshots, repeated undo, audit creation, transaction rollback, literal backslash preservation, sparse metadata sorting, exact query comparisons, screen scoping, native fail-closed filters, unsupported ACF select shapes, warning escaping, and the metadata-filter cost cap.
...
- The latest local HTTP samples have a 0.101 second median and a 0.115 second maximum against a 5 second budget.
